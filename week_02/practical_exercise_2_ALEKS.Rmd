...
The residuals are a bit right-skewed, but it does look alright.
...
The residuals follow a somewhat normal distribution, with a slight right skew (positive skew).

### Results from model
The predictor "gender" has a significant effect (p < 0.05) on mean pitch, showing a decrease from 254.41 Hz for women to 138.96 Hz for men. In comparing polite and informal speech, there is a 14.82 Hz reduction in mean pitch going from the informal to the polite condition (p < 0.05). We also see the random intercepts explaining a good amount of the variance in the dataset (515), especially random intercepts by subject.

```{r}
# Running anova to quickly compare AIC values
anova(m2, m1, m3, m4, m6, m7)
summary(m6)
```