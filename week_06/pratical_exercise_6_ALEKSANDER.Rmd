...
x = np.arange(10) # 10 observations from 1-10
y = 2 * x
y = y.astype(float)
n_samples = len(y)
...
```{python}
# Exercise 1.1.i
#Parameter estimates with matrix calculation
bhat = np.linalg.inv(np.matrix.transpose(X) @ X) @ np.matrix.transpose(X) @ y
print(bhat)

# Estimated values
yhat = bhat[0] + bhat[1]*x
print(yhat)
```
...
import matplotlib.pyplot as plt
plt.figure() # create new figure
plt.plot(x, yhat, 'b-') # plot a blue line
# plt.show() # show figure
plt.plot(x, y, 'ro') # scatter plot (red)
# plt.show()
plt.xlabel('Observed (x)')
plt.ylabel("y")
plt.title('A plot')
plt.legend(['Estimated y-values', 'Observed y-values'])
plt.show()
```
