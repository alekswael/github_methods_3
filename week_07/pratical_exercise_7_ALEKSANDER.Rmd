...
added_y_noise = np.random.normal(0,5,len(x))
y_noise = y+added_y_noise
```
...
regressor.fit(x.reshape(-1,1), y_noise) # Fitting model
y_hat = regressor.predict(x.reshape(-1,1)) # Predicting data based on x
```    
...
# X_quadratic is the second order predictor, so should be a better fit (but an overfit)
```
...
X_penta = penta.fit_transform(x.reshape(-1, 1))
regressor = LinearRegression()
regressor.fit(X_penta, y_noise)
y_penta_hat = regressor.predict(X_penta)
```
